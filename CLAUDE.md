...
原則として、**Match 画面では 1 枚のフルスクリーン UIDocument** を使い、その中を左右 HUD と中央オーバーレイに分割する。
...
- 強化フェーズでは「両プレイヤーの UI を同時に見せる」「中央タイマーを共有する」「完了状態を同期表示する」必要がある。
- パネルを増やしすぎると focus、navigation、sort order、入力配送の複雑さが増える。

したがって、**複数パネル前提で始めず、単一ルートパネル + 左右コンテナで始める**こと。
