...
### Step 4: PlayMode 動作確認（UI・シーン遷移を含む変更時）

UI、シーン遷移、Input System、Presenter の変更を含む場合は、**EditMode テストに加えて PlayMode での動作確認**を行う。

#### 4a. プレイモード起動

```csharp
// Unity MCP RunCommand
EditorSceneManager.OpenScene("Assets/App/Scenes/Boot.unity", OpenSceneMode.Single);
EditorApplication.isPlaying = true;
```

- Boot シーンから起動すること（Title / Match を直接開かない）。
- 起動後 5〜8 秒待ってからシーンロード状態を確認する。

#### 4b. UI フロー検証（ClickEvent シミュレーション）

ボタンクリックは `ClickEvent` + `target` 設定 + `SendEvent` でシミュレーションする。
...
- **注意**: `Clickable.Invoke(null)` や `NavigationSubmitEvent` では `RegisterCallback<ClickEvent>` のコールバックはトリガーされない。必ず上記パターンを使うこと。
- 各画面遷移後に `root.Q("要素名")?.resolvedStyle.display` で表示状態を検証する。

#### 4c. コンポーネント状態確認（Reflection）

SerializeField やランタイム状態は Reflection で確認できる。

```csharp
// 例: TitleLifetimeScope の _inputActions を確認
var go = GameObject.Find("TitleLifetimeScope");
var comp = go.GetComponent("TitleLifetimeScope");
var field = comp.GetType().GetField("_inputActions",
...
```
GetConsoleLogs(logTypes: "error") → 0 件であること
```
...
上記 Step 1〜4 で全件グリーンを確認した後にのみ、commit → push → PR 作成に進む。
...
- UI やシーン遷移を含む変更では、**Step 4 の PlayMode 動作確認も必須**とする。
