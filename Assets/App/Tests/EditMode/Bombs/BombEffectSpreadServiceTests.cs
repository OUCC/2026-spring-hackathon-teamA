...
namespace FloorBreaker.Tests.EditMode.Bombs
{
    ...
    public class BombEffectSpreadServiceTests
    {
        ...
        [Test]
        public void BreakBomb_Distance0_AppliedImmediately()
        {
            var center = new GridPos(5, 5);
            var breakResolver = new BreakBombResolver(_areaResolver);
            var spec = new BombSpec(BombType.Break, 3, 3, 1, 2, 4f, true, 0f, 3f, 5f);
        ...
        [Test]
        public void BreakBomb_Distance1_NotAppliedUntilTick()
        {
            var center = new GridPos(5, 5);
            var breakResolver = new BreakBombResolver(_areaResolver);
            var spec = new BombSpec(BombType.Break, 3, 3, 1, 2, 4f, true, 0f, 3f, 5f);
        ...
        [Test]
        public void BreakBomb_Distance1_AppliedAfterInterval()
        {
            var center = new GridPos(5, 5);
            var breakResolver = new BreakBombResolver(_areaResolver);
            var spec = new BombSpec(BombType.Break, 3, 3, 1, 2, 4f, true, 0f, 3f, 5f);
        ...
        public void FireBomb_SpreadsAtFireInterval()
        {
            ...
            var spec = new BombSpec(BombType.Fire, 3, 3, 1, 1, 2f, false, 3.5f, 0f, 0f);
        ...
        [Test]
        public void BreakBomb_Distance2_RequiresTwoIntervals()
        {
            var center = new GridPos(5, 5);
            var breakResolver = new BreakBombResolver(_areaResolver);
            // 範囲2 のスペック
            var spec = new BombSpec(BombType.Break, 3, 3, 2, 2, 4f, true, 0f, 3f, 5f);
            var result = breakResolver.Resolve(center, spec, _stage);
        ...
        [Test]
        public void DifferentIntervals_FireFasterThanBreak()
        {
            var center = new GridPos(5, 5);

            // 炎ボム (0.15s)
            var fireResolver = new FireBombResolver(_areaResolver);
            var fireSpec = new BombSpec(BombType.Fire, 3, 3, 1, 1, 2f, false, 3.5f, 0f, 0f);
            ...
            // ブレークボム (0.3s)
            var breakResolver = new BreakBombResolver(_areaResolver);
            var breakSpec = new BombSpec(BombType.Break, 3, 3, 1, 2, 4f, true, 0f, 3f, 5f);
            var breakResult = breakResolver.Resolve(center, breakSpec, _stage);
            _service.EnqueueBreakBomb(breakResult, center, new List<PlayerModel>(), null, 0.3f);
        ...
        /// </summary>
        [Test]
        public void BreakBomb_OverlappingWaves_TimerResetKeepsTileCollapsing()
        {
            var breakResolver = new BreakBombResolver(_areaResolver);
            var spec = new BombSpec(BombType.Break, 3, 3, 2, 2, 4f, true, 0f, 3f, 5f);
            ...
            // A を投下 (t=0)
            var resultA = breakResolver.Resolve(centerA, spec, _stage);
            _service.EnqueueBreakBomb(resultA, centerA, players, null, 0.3f);
            ...
            // B を投下 (t=1.3)
            var resultB = breakResolver.Resolve(centerB, spec, _stage);
            _service.EnqueueBreakBomb(resultB, centerB, players, null, 0.3f);
