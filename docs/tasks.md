...
> 目標: Medieval Fantasy SFX Bundle を使った SE 再生基盤を実装し、全 Presenter に SE を統合する。

### T-16.1 SfxIds 定数クラス [Application] ✅
- ファイル: `Assets/App/Shared/Application/Interfaces/SfxIds.cs`
- 20 SE ID を `public const string` で定義

### T-16.2 AudioCatalog ScriptableObject [Infrastructure] ✅
- ファイル: `Assets/App/Shared/Infrastructure/Audio/AudioCatalog.cs`
- sfxId → AudioClip + volume マッピング、Dictionary ルックアップ

### T-16.3 AudioService 実装 [Infrastructure] ✅
- ファイル: `Assets/App/Shared/Infrastructure/Audio/AudioService.cs`
- MonoBehaviour + IAudioService、AudioSource プール (最大8)、ステレオパン対応

### T-16.4 NullAudioService [Infrastructure] ✅
- ファイル: `Assets/App/Shared/Infrastructure/Audio/NullAudioService.cs`
- 無音フォールバック (Match 単体起動用)

### T-16.5 AudioCatalog.asset 作成 ✅
- Medieval Fantasy SFX Bundle から 19 クリップ割り当て

### T-16.6 DI 登録 ✅
- ProjectLifetimeScope: AudioService を子 GameObject から GetComponentInChildren + RegisterInstance
- MatchLifetimeScope フォールバック: NullAudioService 登録

### T-16.7 全 Presenter SE 統合 ✅
- BombPresenter: 発射 + 爆発 (炎/滑落)
- PlayerPresenter: 被弾 + 死亡
- SlimePresenter: スポーン + 攻撃 + 死亡
- StagePresenter: 炎上 + 崩落 + 永久消滅
- UpgradeOverlayPresenter: 購入 + 完了 + ナビゲーション

### T-16.8 フェーズ遷移 SE ✅
- MatchInitializer で MatchClock.CurrentPhase 購読 (縮小/強化/リザルト)

### T-16.9 AudioListener + Dispose 修正 ✅
- SplitScreenCameraSetup: P1 カメラに AudioListener 追加
- MatchInitializer: IDisposable 実装 (UpgradeUI コールバック + フェーズ購読解除)

### T-16.10 テスト修正 ✅
- UpgradeDraftServiceTests: 複数選択対応で Choosing 維持を検証
- UpgradePhaseUseCaseTests: Skip 経由の完了を検証
- 244/244 全件グリーン
