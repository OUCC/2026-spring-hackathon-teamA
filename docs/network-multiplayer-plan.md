...
ローカル対戦（2〜4P + CPU 混在）のゲーム体験を維持したまま、オンライン N 人対戦（2〜4P）に対応する。
ローカルと同様に、オンラインでも人数選択・CPU 混在を可能にする。
...
| `MatchClock` | `SetPhase(GamePhase)` は既存。`ResetTimer(float)` も既存。追加変更は不要 | クライアント側で時計を同期するため |
...
| プレイヤー位置 ×N | ホスト確定 / クライアント予測 | `[Networked]` + 補間 | 0.2 秒間隔の離散移動のため予測なしも検討可 |
| プレイヤー HP・コイン ×N | ホスト | `[Networked]` | チート防止 |
| プレイヤー向き ×N | ホスト | `[Networked]` | |
| ボム飛行状態 ×N | ホスト | `[Networked]` | 飛行開始/着弾をイベントで送信 |
| ボムクールダウン ×N | ホスト | `[Networked]` | |
...
Tick 順序は `InputBridge.Tick()` → `Scheduler.Tick(dt)` → `Presenters.Tick(dt)` → `Camera.Tick(dt)` で決定的に制御されている。
フレームレートが異なるクライアント間で `deltaTime` が異なるため、オンライン対戦では固定 Tick への移行が必要。
...
合計: 約 4 byte/プレイヤー。グリッドゲームの強み。
Fusion 2 はプレイヤーごとに `GetInput()` を呼ぶため、この構造体を N 人分そのまま使える。
...
現在の `GameplayInputBridge` はホールドリピート移動（`InputBufferTime`, `InitialRepeatDelay`）とダッシュ処理を実装している。
`PlayerInputAdapter` は `ITimeProvider` 経由でダブルタップ検出の時刻を取得しており、ネットワーク環境でも時刻基準を統一できる。
...
各プレイヤーごとに `NetworkPlayerState` を 1 つ生成する（最大 4 つ）。`PlayerId.Index` で対応付け。
...
`SlimeId` の生成は `SlimeSpawnService` のインスタンスカウンタで管理されており、静的状態に依存しない。
ホストが ID を生成し、RPC でクライアントに配布する構造と自然に整合する。
...
N 人分の候補生成・選択・完了待ちが必要。既存の `UpgradePhaseUseCase` はプレイヤーリストをループで処理し、`IsComplete` は全員の `DraftState` を確認するため、N 人にそのまま対応する。

```
ホスト:
  UpgradePhaseUseCase.Start(players, random)
    → 各プレイヤーに RNG で候補生成
    → RPC: UpgradeChoicesGenerated(PlayerId, UpgradeDefinition[3]) × N 人
...
現在のタイトル画面は `MatchModeConfig` により人数（2〜4P）とCPUスロット配置を柔軟に設定できる構造。

```
タイトル画面
  ├─ 「ローカル対戦」(2〜4P) → ネットワーク無し、従来フロー
...
現在のタイトル画面はリデザイン済みで、人数選択・CPU 設定が統合されている。

```
タイトル画面:
  既存: 人数選択 (2〜4P)、CPU スロット ON/OFF、マッチ開始
  追加: 「オンライン対戦」モード選択
         → 「部屋を作る」ボタン
         → 「部屋に入る」ボタン + ルームコード入力欄
```
...
Assets/App/
  Features/
    Network/                              ← 新規 Feature
      ...
      Application/
        MatchMode.cs                      ← Local / Cpu / Online enum（既存 MatchModeConfig を拡張）
...
**問題:** いずれかのプレイヤーの選択入力がロストし、タイムアウトまで進行しない。
...
> **進捗: 大部分が完了済み。** 残りは Domain セッター追加と入力ソース抽象化のみ。

1. **Domain 層への最小限のセッター追加** (§1.5)
   - `PlayerStats`: `SetHpMirror(int)`, `SetCoinsMirror(int)` — **未実装**
   - `PlayerBuild`: `ApplySnapshot(...)` またはプロパティ直接セッター — **未実装**（現在は public setter あり）
   - `MatchClock`: `SetPhase(GamePhase)` / `ResetTimer(float)` — **実装済み**
   - `StageModel`: `LoadSnapshot(TileData[,])` — **未実装**
   - `GridPos`: `INetworkStruct` 実装、または Infrastructure に `NetworkGridPos` 変換型を追加 — **未実装**
   - これらは `internal` とし、`InternalsVisibleTo` で Network asmdef のみに公開

2. **Tick ソースの抽象化** — **完了**
   - `MatchTickRunner` は `ITimeProvider.DeltaTime` を使用（`Time.deltaTime` 直接依存なし）
   - Tick 順序は `InputBridge → Scheduler → Presenters → Camera` で決定的に制御済み

3. **入力ソースの抽象化** — **未完了**
   - `IInputSource` インターフェースの導入が必要
   - `GameplayInputBridge` を `LocalInputSource` としてラップ
   - CPU AI も `IInputSource` 実装にリファクタ

4. **MatchMode 設定の整備** — **大部分完了**
   - ~~`MatchModeSelection` の static mutable state を解消~~ → **完了**: `MatchModeConfig` (sealed class, Singleton 登録) に置き換え済み
   - `MatchModeConfig` は `PlayerCount` と `IsCpuSlot[]` を持ち、2〜4P + CPU 混在を柔軟に設定可能
   - `MatchLifetimeScope` が `MatchModeConfig` に応じて DI を切り替え済み
   - `MatchMode` enum (`Local` / `Cpu` / `Online`) は未導入 — オンライン対応時に追加

5. **シーン遷移の抽象化** — **完了**
   - `ISceneTransitionService` は既に存在（`LoadMatchAsync()` / `LoadTitleAsync()`）
   - ネットワーク実装 (`Runner.LoadScene()`) は Phase 1 で追加

6. **決定論性の阻害要因の除去** — **完了**
   - ~~`SlimeId.Next()` の静的カウンタ~~ → `SlimeSpawnService` のインスタンスフィールドに移動済み
   - ~~`CpuPlayerBrain` の `System.Random`~~ → `IRandomProvider` の DI 注入に置換済み
   - ~~`PlayerInputAdapter` の `Time.unscaledTime` 直接参照~~ → `ITimeProvider.UnscaledTime` 経由に変更済み

7. **N 人対応の基盤** — **完了**
   - `PlayerId` は 4 人対応済み (`MaxPlayers = 4`)
   - `PlayerInputAdapter.BindActions()` は動的 `$"Gameplay_P{_playerId.Index + 1}"` を使用
   - `InputMapSwitcher` はプレイヤー数ベースの動的ループ
   - カメラは 1〜4 分割対応済み (`SplitScreenCameraSetup`)
   - UI ペイン構造は N 人対応済み
...
- `FloorBreakerInput`: 約 4 byte × 30 Tick/s = **120 byte/s** (per client)
- Fusion ヘッダオーバーヘッド: 約 8-12 byte/packet → 実質 **360-480 byte/s** (per client)

### 11.2 状態同期 (ホスト → 各クライアント)

Fusion 2 はデルタ圧縮を行うため、変更のないプロパティは送信されない。
以下は **4P 時**の**変更頻度ベース**の実効見積もり（2P 時は約半分）:

| データ | サイズ | 実効変更頻度 | 実効 bps | 備考 |
|--------|--------|------|-----|------|
| プレイヤー状態 ×4 | 約 50 byte ×4 | 5-10 Hz (変更時のみ) | 1,000-2,000 | 移動は 5 Hz、HP/コイン変更は稀 |
| ボム飛行イベント ×4 | 約 20 byte | 数回/秒 | 約 200 | |
| タイル変更バッチ RPC | 約 5 byte × N | イベント時 | 約 300 | 4P だとボム爆発頻度増 |
| スライム位置 ×27 | 約 220 byte | 2.5 Hz (移動時のみ) | 550 | 移動速度がプレイヤーの半分 |
| フェーズ・タイマー | 約 8 byte | 30 Hz | 240 | |
| 全盤面スナップショット | 900 byte | 0.2 Hz | 180 | |

**合計: 4P 時 約 4-6 KB/s (通常時)、バースト時 8-15 KB/s** — 十分軽量。
2P 時は約 2-3 KB/s。Fusion プロトコルオーバーヘッド (+30-50%) を加算しても **6-20 KB/s** に収まる。
...
| Phase | 検証 |
|-------|------|
| 0 | 既存 EditMode テスト全件グリーン。ローカル対戦（2〜4P）が壊れていないこと |
| 1 | 2〜4 クライアントが同一ルームに接続・切断できること |
| 2 | 全プレイヤーの移動・ボム発射がホスト側で正しく処理されること |
| 3 | 各クライアント側で正しい盤面・HP・コインが表示されること |
| 4 | N 人の強化フェーズの選択・タイムアウトが正しく動作すること |
...
- **オンラインは 2〜4P 対応**（ローカルと同じ人数・CPU 混在構成をそのまま使用）
...
| 13 | `MatchModeSelection` static mutable state | Phase 0 で解消済み（`MatchModeConfig` に置き換え） |
...
| 日付 | 内容 |
|------|------|
| 初版 | Fusion 2 Host Mode 移行計画を策定。レビュー指摘 14 件を反映 |
| 2026-03-29 | Phase 0 進捗を反映。完了済み項目: Tick 抽象化、MatchModeConfig 移行、ISceneTransitionService、N 人基盤。決定論性阻害要因 3 件を修正（SlimeId 静的カウンタ除去、CpuPlayerBrain を IRandomProvider 化、PlayerInputAdapter を ITimeProvider 化）。MatchClock API 名を実装に合わせて修正（`SetRemaining` → `ResetTimer`）。4P 対応済みの構造を各所に反映 |
| 2026-03-29 | オンライン対戦の対象を 1v1 → N 人（2〜4P）に拡張。権威モデル表、入力設計、帯域見積もり、強化フェーズ、テスト戦略を N 人対応に更新。ローカルと同じ人数・CPU 混在構成をオンラインでも使用する方針に変更 |
